...
public class Save_canvas : MonoBehaviour
{
    /// <summary>
            ...
            switch (Sim)
            {
                case SimDevice.iPhoneX:
                    if (Screen.height > Screen.width)  // Portrait
                        nsa = this.NSA_iPhoneX[0];
                    else  // Landscape
                        nsa = this.NSA_iPhoneX[1];
                    break;

                case SimDevice.iPhoneXsMax:
                    if (Screen.height > Screen.width)  // Portrait
                        nsa = this.NSA_iPhoneXsMax[0];
                    else  // Landscape
                        nsa = this.NSA_iPhoneXsMax[1];
                    break;

                case SimDevice.Pixel3XL_LSL:
                    if (Screen.height > Screen.width)  // Portrait
                        nsa = this.NSA_Pixel3XL_LSL[0];
                    else  // Landscape
                        nsa = this.NSA_Pixel3XL_LSL[1];
                    break;

                case SimDevice.Pixel3XL_LSR:
                    if (Screen.height > Screen.width)  // Portrait
                        nsa = this.NSA_Pixel3XL_LSR[0];
                    else  // Landscape
                        nsa = this.NSA_Pixel3XL_LSR[1];
                    break;
    ...
    private void ApplySafeArea(Rect r)
    {
        ...
        if (this.Logging)
        {
            ...
            this.name, r.x, r.y, r.width, r.height, Screen.width, Screen.height);
        }
    }
}
